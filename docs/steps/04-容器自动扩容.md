...
### 8.3 子节点移出容器（Ctrl 键交互）

#### 8.3.1 交互需求
用户需要能够将子节点从容器中移出，但由于容器会自动扩容包裹子节点，需要提供明确的交互机制。

#### 8.3.2 方案选择
**采用方案：Ctrl 键辅助移出**

经过评估，方案 A（基于重叠率）在固定左上角的扩容策略下存在根本性矛盾（容器持续扩展导致子节点无法脱离），因此采用方案 C（Ctrl 键）。
...
```typescript
1. Ctrl 键状态管理（useKeyboardState）：
   - 监听全局 keydown/keyup
   - 维护 isCtrlPressed 响应式状态
   - 处理窗口失焦自动重置

2. 扩容控制增强（useAutoExpand）：
   - 新增 pause() 方法：暂停扩容
   - 新增 resume() 方法：恢复扩容
   - 新增 isPaused 状态标志

3. 出组判定（useNodeOutGroup）：
   - 监听 node:mouseup 事件
   - 检查 Ctrl 状态 + 重叠率
   - 重叠率 = 0% 时调用 removeChild()
   - 出组后保持节点当前位置（绝对坐标不变）

4. 重叠率计算：
   - 使用现有 coordinateTransform.ts 工具
   - 阈值：0%（严格模式，完全脱离才出组）

5. 位置保持机制：
   - 出组前记录节点当前位置（绝对坐标）
   - 调用 removeChild() 后立即恢复该位置
   - 确保视觉上节点保持在鼠标松开的位置
```
...
export const OUT_GROUP_CONFIG = {
  modifierKey: 'Ctrl',        // 快捷键
  overlapThreshold: 0,         // 重叠率阈值（0% = 完全脱离）
  highlightColor: '#FFC107',   // 高亮颜色
  warningColor: '#F44336',     // 警告颜色
...
#### Q5: Ctrl + 拖动出组后节点消失？
**A**: 检查 `removeFromContainer()` 方法是否在 `removeChild()` 后调用了 `setPosition()` 恢复节点位置。出组前必须保存节点当前的绝对坐标，然后在出组后立即恢复该位置，确保节点显示在鼠标松开的位置。
