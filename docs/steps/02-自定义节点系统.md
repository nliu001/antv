...
### 1.2 前置依赖
- ✅ Step 1：基础画布已搭建完成
- ✅ 已安装 `@antv/x6-vue3-shape` 依赖（Vue 3 版本）
- ✅ Graph 实例已可用

**重要说明**：
- X6 3.x + Vue 3 必须使用 `@antv/x6-vue3-shape`（注意有 `vue3` 标识）
- `@antv/x6-vue-shape` 仅支持 Vue 2，不能在本项目中使用
...
|---------|---------|--------|----------|------|
| T2-1 | 安装 @antv/x6-vue3-shape 依赖 | P0 | 1h | 无 |
...
#### 8.1.1 在应用启动时引入 Vue3 Shape
在 `main.ts` 中导入 `@antv/x6-vue3-shape` 包（必须在创建 Graph 之前）：

```typescript
// main.ts
import { createApp } from 'vue'
import '@antv/x6-vue3-shape'  // ⭐ 必须导入，初始化 Vue3 Shape 支持
